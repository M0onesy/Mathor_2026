...
剔除血脂四项 $\to$ WOE-Logistic 评分卡 $\to$ GBDT/RF 对照 $\to$ 多准则分层与校准验证 \\
\hline
$\downarrow$ \\
\hline
\textbf{Step 4 · 问题三 · 6 月干预优化} \\
双目标 Pareto $\min(S_6,C_{\text{total}})$ $\to$ 月度前沿推进 $\to$ Extended Dominance 预筛选 $\to$ NMB($\lambda$) 统一选点 $\to$ 稳健性与 TOPSIS/Kneedle 对照 \\
\hline
\end{tabularx}
\end{center}
...
\textbf{Borda 计数法}\cite{saari1995borda}是社会选择理论中的经典位置得分聚合方法。对每种方法的得分排序，排第 1 位给 $n$ 分，第 2 位给 $n-1$ 分，末位给 1 分。最终每个特征的总分 $=$ 它在所有方法排名中的得分之和。其优势：
...
\item \textbf{秩次聚合稳健}：用位置得分聚合多个排序，避免直接比较不同量纲、不同分布形态的原始得分，但不额外声称满足 Condorcet 准则；
...
\subsubsection{初稿规则化预警方案的症结}

在初稿中曾采用由临床阈值与经验加权组成的规则化预警体系，其临床语义清晰但方法论上存在三个根本缺陷：
\begin{enumerate}
\def\labelenumi{(\arabic{enumi})}
\tightlist
...
\item \textbf{代谢关键变量未入模}：问题一已指出血尿酸位列发病风险 Top-1（Borda 得分 65）、空腹血糖亦稳健入选，但初稿规则化方案仅纳入 BMI，\textbf{空腹血糖、血尿酸完全未用}，构成建模信息的重大遗漏。
...
\textbf{步骤 3}~ 多准则阈值（Youden + Sp$\ge$0.70 + F1 + 成本敏感 + Bootstrap BCa）\\ \hline
...
\end{itemize}
\textbf{UA 信号的可靠性边界}：UA 在诊断规则中未出现（规则仅涉及 TC/TG/LDL-C/HDL-C 四项阈值），故其高 IV 不属于直接标签泄漏。但 $\mathrm{IV}=2.158$ 已远超信用评分实践中 $0.5$ 的警戒线，说明其区分能力强到需要谨慎解释：除真实代谢机制外，也不排除样例数据生成过程引入了 UA 与血脂标签之间的间接规则关联。因此本文把 UA 作为当前数据下的核心筛查信号，同时要求在外部真实队列中验证该 U 型关系是否保持。
...
原拟取 Sp$\ge 0.95$ 作为 $c_L$，但本数据阳性率高达 $79.3\%$（阴性组仅 $207$ 人），使 Sp$\ge 0.95$ 所要求的``阴性中仅允许 $\le 10.4$ 人误判''约束\textbf{在当前概率/评分分布下不可达}（退化为 $c_L$ 与 $c_H$ 几乎重合，中风险层样本为 $0$）。因此本文改用更宽松的 Sp$\ge 0.70$ 目标作为 $c_L$，对应低危组假阳性率约 $30\%$，与弗拉明汉低危组定义一致（2008 FRS 低危组假阳 $\approx 20\text{--}35\%$）。该放宽使三层样本分布均匀，结果见下节。临床应用时，低危组仅表示当前筛查分层较低，不能视作完全免检或绿灯放行，仍建议结合常规体检周期进行定期随访。
...
\midrule
\textbf{模型扰动}（Cohen's $\kappa$ 分层一致性）
...
\subsection{与初稿规则化预警方案的量化对比}\label{sec:q2-compare}

\begin{table}[H]
\centering\small
\caption{本文方案 vs.~初稿规则化预警方案的系统对比}\label{tab:q2-compare}
\begin{tabular}{@{}l l l@{}}
\toprule
维度 & 原方案 & 本文方案 \\
\midrule
...
\textbf{规则 B 的文字边界}。``每提升一级活动干预强度下降 3\%''也可被理解为相对 $k=1$ 基线的增量，即 $0.03(k-1)$。本文采用 $0.03k$ 的理由是附表 3 已把 $k=1,2,3$ 三档均列为有效活动强度，而非另设 $k=0$ 或``不活动''档；同时该读法与 $f=5,k=3$ 时 $9\%$、$f=10,k=3$ 时 $14\%$ 的最大月降幅范围一致。为避免过度断言，后文用规则系数 $\pm20\%$ 扰动检验该解释下推荐结果与策略迁移方向的稳健性。
...
由于复利递推后 $S_t$ 通常为连续值，本文直接按实数阈值分档：$S_t<59$ 归入 $L=1$，$59\le S_t<62$ 归入 $L=2$，$S_t\ge62$ 归入 $L=3$。这等价于对题目附表 2 的整数区间 $0$--$58$、$59$--$61$、$\ge62$ 做连续化延拓，避免 $58$ 与 $59$ 之间出现未定义空隙。
...
\item \textbf{月度扩展}: 对 $t=1,\dots,6$, 对前沿 $\mathcal{F}_{t-1}$ 中每个状态枚举 $(k_t, f_t) \in K_i\times\{5,\dots,10\}$, 计算 $L=g(S_{t-1})$、$r_t = 0.03 k_t + 0.01(f_t-5)$、$S_t=S_{t-1}(1-r_t)$、$\Delta C=4f_t c_{\text{act}}(k_t)+c_{\text{中医}}(L)$, 若 $C_t \le 2400$ 则并入新前沿。这里 $2400$ 元仅为枚举阶段的安全缓冲上界, 用于避免 $2000$ 元软预算附近的方案被过早剪枝; 最终推荐仍由 Pareto 前沿与 NMB 准则决定。
...
\mathrm{NMB}_B
&= \lambda E_B - C_B \\
&\le \lambda\bigl[\alpha E_A + (1-\alpha)E_C\bigr]
   - \bigl[\alpha C_A + (1-\alpha)C_C\bigr] \\
&= \alpha\,\mathrm{NMB}_A + (1-\alpha)\,\mathrm{NMB}_C \\
&\le \max(\mathrm{NMB}_A, \mathrm{NMB}_C).
\end{aligned}
\end{equation}
\hfill$\blacksquare$
...
\begin{equation}
\;\mathrm{NMB}_j(\lambda)\;=\;\lambda\,\underbrace{(S_0 - S_6^j)}_{E_j:\,效果}\;-\;\underbrace{C_{\text{total}}^j}_{成本}\;
\label{eq:q3-nmb}
\end{equation}
...
\begin{equation}
\begin{aligned}
\Delta\mathrm{NMB}_{j\to j+1}
&= \mathrm{NMB}_{j+1}(\lambda) - \mathrm{NMB}_j(\lambda) \\
&= \lambda(E_{j+1}-E_j) - (C_{j+1}-C_j) \\
&= (E_{j+1}-E_j)\bigl(\lambda - \mathrm{ICER}_{j,j+1}\bigr).
\end{aligned}
\end{equation}
...
若三种方法在 $>80\%$ 患者上一致, 则 NMB 准则对方法论选择具有鲁棒性; 若不一致, 则披露 $\lambda$ 在决策中的关键作用。本数据结果: 三位样本中 NMB($\lambda=30$) 与 TOPSIS 推荐均不重合，TOPSIS 分别给出 $(S_6,C)=(50.56,700)/(47.81,552)/(48.13,614)$ 的更低成本方案；NMB 与 Kneedle 在样本 2/3 的几何拐点上较接近，但样本 1 仍因 $\lambda=30$ 超过其末端 ICER 而选取最高效果点。差异恰好体现了 TOPSIS 对``距理想解近''的偏好、Kneedle 对几何拐点的偏好与 NMB 对``成本-效果兑换率''的偏好之分, 证实 NMB 作为\emph{唯一主准则}的不可替代性。
...
\textbf{决策叙事}: NMB 选择\textbf{"首月低负荷缓冲 + 后 5 月冲刺"的精巧组合}——首月 $(k=1, f=5)$ 月降 3\% 将 $S$ 从 59 降到 57.23, 已足以跨越 $L=2\to L=1$ 档位阈值; 第 2--6 月立即启动 $f=10$ 月降 8\% 集中冲刺至 $S_6=37.72$。需要强调的是，首月 $f=5$ 与 $f=10$ 均会使第 2 月起进入 $L=1$，两者中医成本相同；差异只在活动费与终点效果：$f=5$ 比首月 $f=10$ 节省 60 元活动费，代价是 $S_6$ 约高 1.95 分。在 $\lambda=30$ 下，额外降分的货币价值约 $30\times1.95=58.5$ 元，小于 60 元活动费节省，故 NMB 准则偏好首月 $f=5$ 的省钱方案。总成本 $\mathbf{890}$ 元 (首月 140 + 后 5 月 150$\times$5), 降幅 $\mathbf{36.1\%}$, 平均 CER $=890/21.28=41.8$ 元/分。值得注意: 样本 3 的 NMB@$\lambda=30$ 方案\textbf{并未启用 $k=2, k=3$ 最高强度}——因为 $\lambda=30$ 低于 $k=1\to 2$ 边际 ICER ($\sim 69$) 与 $k=2\to 3$ 边际 ICER ($\sim 121$), NMB 严格拒绝付出不值得的强度升级成本。
...
\begin{tabularx}{\linewidth}{@{}c c c r L r r r r@{}}
\toprule
样本 & $K_i$ & $S_0$ & ED 前沿点数 & 核心策略 & $S_6$ & 总成本 & 降幅 & CER \\
\midrule
1 & $\{1\}$     & 64 & 26 & 首月高频冲档；后 5 月持续高频 & 38.81 & 1000 & 39.4\% & 39.7 \\
2 & $\{1,2\}$   & 58 & 37 & $k=1$；中间 1 月缓冲，其余月份高频 & 37.08 &  840 & 36.1\% & 40.2 \\
3 & $\{1,2,3\}$ & 59 & 43 & 首月 $f=5$ 省活动费；后 5 月高频冲刺 & 37.72 &  890 & 36.1\% & 41.8 \\
\bottomrule
\end{tabularx}
\end{table}
...
\item \textbf{$S_0$ 相对 TCM 档位位置} (机制 2): 样本 1 ($S_0=64, L=3$) 需首月 $f=10$ 冲破 $L_1=3\to L=1$ 节省 100 元/月; 样本 2 ($S_0=58, L=1$) 已在最低档, 只需选缓冲月节省活动费; 样本 3 ($S_0=59, L=2$) 首月 $f=5$ 已可降至 57.23 并进入 $L=1$，相比首月 $f=10$ 的主要优势是节省 60 元活动费，而不是额外节省中医费——\emph{首月 $f$ 的选择同时受 $L$ 阈值和 $\lambda$ 成本-效果兑换率锚定};
...
\textbf{批量结果来源}: 以下表格由修订后统一 NMB 流程直接批量计算得到，代码输出见 \texttt{Q3\_batch\_results.csv} 及按 $|K_i|$、$S_0$ 档位、年龄组、成本档位聚合的配套表。
...
\midrule
1 & 92 & 37.90 & 906.41 & 36.78 \\
2 & 171 & 37.78 & 900.18 & 36.68 \\
3 & 15 & 37.72 & 891.33 & 36.46 \\
\bottomrule
\end{tabular}
\label{tab:q3-by-K}
\end{table}
...
2 档(59--61) & 88 & 37.62 & 911.82 & 37.27 & 40.82 \\
3 档($\ge 62$) & 90 & 38.27 & 1005.89 & 39.48 & 40.30 \\
\bottomrule
\end{tabular}
\label{tab:q3-by-S0}
\end{table}
...
\midrule
低档 ($C_\text{total} \le 500$ 元) & 0 & --- & --- & --- & --- & --- \\
中档 ($500 < C_\text{total} \le 1000$ 元) & 269 & 59.69 & 37.79 & 896.51 & 36.58 & 41.05 \\
高档 ($C_\text{total} > 1000$ 元) & 9 & 65.00 & 38.65 & 1058.89 & 40.54 & 40.19 \\
\bottomrule
\end{tabular}
...
\def\labelenumi{(\arabic{enumi})}
\tightlist
\item \textbf{在 $\lambda=30$ 基准下，推荐方案集中于保守有效区间}。三类可行集的平均 $S_6$ 均约为 $37.7$--$37.9$，平均成本约 $891$--$906$ 元，说明 NMB 准则在低 WTP 情境下主要选择 $k=1$ 与 $f=5/10$ 的组合，而不是简单把可行强度用满。
\item \textbf{$S_0$ 相对阈值位置仍影响成本投入}。1/2/3 档平均成本分别为 $799.20/911.82/1005.89$ 元，平均降幅由 $33.71\%$ 升至 $39.48\%$，显示初始积分越高，越容易触发前期冲档或持续高频以降低后续状态负担。
\item \textbf{年龄约束在基准 $\lambda$ 下不再造成明显断层}。40--89 岁五组平均降幅均在 $36.3\%$--$37.0\%$ 之间，平均 CER 约 $41$ 元/分，说明 NMB@$\lambda=30$ 的保守策略没有强行推高高龄组强度，反而让不同年龄组保持了相近性价比。
\item \textbf{成本分布高度集中}: 278 人中 $269$ 人落在 $500$--$1000$ 元的中档，9 人略高于 1000 元，低于 500 元者为 0。该结果表明在当前模型和 $\lambda=30$ 下，统一 NMB 准则自然给出一组稳定、可支付且低于 2000 元建议上限的社区干预方案。
\end{enumerate}
...
\caption{问题三 278 名痰湿体质患者的 NMB@$\lambda=30$ 推荐方案结果汇总。(a) $S_0$-$S_6$ 散点按 NMB 推荐总成本着色; (b) 总成本主要集中在 800--1000 元区间, 全部低于 2000 元建议上限; (c) 降幅集中在约 34\%--40\%; (d) 按 $|K_i|$ 分组的降幅箱线图差异较小, 说明基准 $\lambda$ 下 NMB 未盲目用满高强度可行集; (e) 按年龄组的总成本总体接近; (f) 按 $S_0$ 档位的 CER, 三档性价比基本稳定。}\label{fig:q3-summary}
\end{figure}
...
\midrule
1 & $40.95 \pm 5.27$ & $[34.93,\,48.79]$ & $[810,\,1050]$ & $38.81/1000$ & $k=1$ 保持 $9/9$；$f=10$ 全程占 $6/9$，低 rule\_C 时出现 $f=5$ 缓冲。 \\
2 & $37.69 \pm 5.06$ & $[31.65,\,48.08]$ & $[600,\,900]$ & $37.08/840$ & $k=1$ 占 $6/9$，$k=2$ 或后期切到 $k=2$ 占 $3/9$；$f$ 在 $5/10$ 间迁移。 \\
3 & $37.75 \pm 4.46$ & $[32.20,\,46.90]$ & $[710,\,950]$ & $37.72/890$ & $k=1$ 占 $6/9$，$k=2$ 或后期切到 $k=2$ 占 $3/9$；$f$ 在首月缓冲、全程高频与 $k=2$ 低频替代之间迁移。 \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{稳健性结论}: 在规则系数 $\pm 20\%$ 扰动的 $9\times 3 = 27$ 组情景下, 三位样本的基准推荐点均落在对应敏感性区间内, 且位置接近各自的中位数或中心区间。最优方案并非``完全不变'', 而是会在低频缓冲、全程高频和 $k=2$ 低频替代之间发生可解释迁移: 当 rule\_C 偏低时, 高频边际收益下降, NMB 更倾向插入 $f=5$ 缓冲月; 当 rule\_B 偏高而 rule\_C 偏低时, 样本 2/3 会转向 $k=2,f=5$ 一类更省钱的强度替代方案; 当两类系数同时偏高时, 全程 $f=10$ 再次占优。这说明本文坚持题目最小假设集后, 参数不确定性仍然可以被限定在\textbf{方向清晰、机制可解释}的范围内。
...
\begin{itemize}
\tightlist
\item 样本 1: 预算 $1000$ 元恰好覆盖 NMB@$\lambda=30$ 推荐方案 ($1000$ 元), 再升至 2000 元仅使 $\lambda \ge 50$ 档位的同一全程高频方案继续保持可行;
\item 样本 2: 预算 $1000$ 元已覆盖 NMB 推荐 ($840$ 元), 在此之前预算过紧时只能选更保守的低成本方案; 预算进一步放宽至可承受 $1380$ 元后, $\lambda=100$ 才会激活 $k=2$ 升级;
\item 样本 3: 预算 $1000$ 元已覆盖 NMB 推荐 ($890$ 元); 政策 $\lambda=200$ 激活 $k=3$ 全程 ($2150$ 元) 轻微超过 $2000$ 元软约束, 可通过$\lambda=150$ 等中间值实现"几乎 $k=3$ 但控制在 2000 元内"的折衷。
\end{itemize}
...
\def\labelenumi{(\arabic{enumi})}
\tightlist
\item \textbf{统一准则取代主观分类}: 以单一净货币收益公式 $\mathrm{NMB}(\lambda)=\lambda(S_0-S_6)-C_{\text{total}}$ 的 $\arg\max$ 处理所有患者, 彻底消除原``经济型/肘点型/冲档型''三规则的人为边界, 患者异质性完全由可行集 $K_i$ 与起点 $S_0$ 的数学差异从统一公式中\emph{自发涌现}——三位样本在 $\lambda=30$ 基准下分别得到 $(S_6,C)=(38.81,1000)/(37.08,840)/(37.72,890)$, 无任何 if-else 分支。
...
\item \textbf{问题三}：(a) 可结合真实随访 RCT 数据对规则系数 $3\%/1\%$ 进行患者异质性标定; (b) 若未来医学研究给出中医调理本身的降分率参数, 可以在保持模型结构的前提下把 $r_{\text{中医}}$ 项纳入状态递推 (本文因题目未提供此参数严格不作推测); (c) 可将当前二维 Pareto 前沿 $(S_6, C_{\text{total}})$ 扩展为加入``训练时长''维度的三目标前沿, 为时间紧张的患者提供更多决策维度; (d) 可在动力学中引入正态扰动将模型推广为随机优化, 用 CVaR 等风险度量替代期望值, 提高方案对参数不确定性的抵抗力; (e) 若获得真实随访数据, 可用强化学习替代 DP 学习个体响应函数实现自适应闭环干预。
\end{enumerate}
