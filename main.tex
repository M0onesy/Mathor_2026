...
\textbf{问题一}：严格按题意限定候选特征为\textbf{血常规指标（7 项）+ 活动量表评分（ADL/IADL 子项 10 项）}，共 17 个指标；排除性别、吸烟、饮酒、年龄组等非题设范围变量。先进行\textbf{多重共线性诊断（VIF）}剔除 $\text{ADL总分}=\Sigma \text{ADL}_i$ 这类完全共线的汇总项，然后采用 5 种互补方法\textbf{（Spearman 秩相关、互信息、LASSO 稀疏回归、随机森林置换重要度、偏相关）}；方法选取的设计依据是``参数／非参数 $\times$ 线性／非线性 $\times$ 单变量／控制其它变量''的三维正交覆盖。聚合方式从 Z-score 加和升级为 \textbf{Borda 排序聚合}，规避量纲与分布偏态带来的偏差。结果：表征痰湿体质严重程度最优 Top-5 指标依次为 \textbf{TC、ADL 吃饭、BMI、TG、ADL 洗澡}；预警高血脂发病风险最优 Top-5 指标为 \textbf{TG、TC、血尿酸、HDL-C、LDL-C}（TG Borda 得分 84，TC 得分 80，显著领先）。

\textbf{九种体质贡献度}：采用 Borda 聚合五项指标（Cohen's $h$、RR、ARR、独热 $\beta$、积分多元 $\beta$）。发现本数据中各体质发病率差异并不显著（74.0\%--86.3\%），阳虚质发病率最高（86.3\%），而\textbf{痰湿体质发病率 77.3\% 反而低于整体均值 79.3\%}。真正的统计信号在于\textbf{剂量--响应关系}：痰湿积分 $\le 20$、$20$--$40$、$40$--$60$、$\ge 60$ 四组发病率分别为 78.6\%／82.3\%／79.0\%／73.8\%，未呈现单调递增。结合 Cohen's $h = 0.115$（痰湿质 vs.~其他体质），痰湿体质的贡献\textbf{更多体现于痰湿积分作为体质量化标尺的描述性意义}，而非二分类标签的直接预测力。
...
\textbf{问题一}要求\textit{从血常规体检指标和中老年人活动量表评分中}，筛选出能表征痰湿体质严重程度、并预警高血脂发病风险的关键指标，研究九种体质的风险贡献度差异。注意题目严格限定了候选特征的范围（血常规 + 活动量表），不应纳入性别、吸烟、饮酒、年龄组等基础信息作为筛选对象。本质上这是一个 \textbf{受约束特征集合下的多重假设检验与方法集成排序问题}，且活动量表应尽量利用 ADL/IADL 各子项以保留信息，同时须排除冗余汇总项带来的多重共线性。
...
\item 体质积分采用王琦九体质量表标准，各积分越高体质偏颇越显著。
...
严格按题目要求，候选特征仅来源于``血常规体检指标''与``中老年人活动量表评分''两类，具体：
...
\textbf{不纳入}性别、年龄组、吸烟史、饮酒史等基础信息作为筛选对象（这是旧版本的错误之一）。
...
旧版本使用的 6 种方法存在冗余（如 F 统计量与点二列相关本质上是同类均值差异检验；Spearman 与点二列都反映线性单调关系），且 Z-score 加和对量纲差异与尾部敏感。本文重新设计 \textbf{5 种互补方法}，在``参数／非参数 $\times$ 线性／非线性 $\times$ 单变量／控制其它''三维正交覆盖：
...
M3 LASSO$|\beta|$       & 参     & 线性     & 控制其它 & 稀疏特征筛选,内生处理共线 \\
...
\item \textbf{M3 LASSO $|\beta|$}：以 $\ell_1$ 罚项 $\lambda \|\beta\|_1$ 做回归，交叉验证选 $\lambda$，$\beta_j = 0$ 的特征被自动剔除，\textbf{内生处理多重共线性}；
...
\subsubsection{Borda 排序聚合（替代 Z-score 加和）}

旧版采用 Z-score 加和，存在两个根本问题：(1) Z-score 受\textbf{分布偏态}影响剧烈（如 F 统计量分布严重右偏）；(2) 不同方法\textbf{量纲不同}（MI 是 nats，LASSO $\beta$ 是标准化系数），即使 Z-score 也无法完全消除量纲差异。
...
\toprule
排名 & 指标 & Spearman$|r|$ & MI & LASSO$|\beta|$ & RF perm & 偏相关 & \textbf{Borda} \\
\midrule
1  & \textbf{TC（总胆固醇）} & 0.035 & 0.000 & 0.000 & 0.125 & 0.043 & \textbf{73} \\
2  & \textbf{ADL 吃饭}     & 0.079 & 0.000 & 0.443 & 0.136 & 0.073 & \textbf{72} \\
...
\item \textbf{LASSO 几乎全部系数为 0}（仅 ADL 吃饭 $|\beta|=0.443$），说明痰湿积分的变化主要由单个``运动能力''信号主导，其他指标与痰湿积分的线性关系微弱，故非线性方法（MI、RF）贡献更大。
\end{itemize}
...
\begin{tabular}{@{}c l c c c c c c@{}}
\toprule
排名 & 指标 & Spearman$|r|$ & MI & LASSO$|\beta|$ & RF perm & Wald & \textbf{Borda} \\
\midrule
1  & \textbf{TG（甘油三酯）}  & 0.489 & 0.180 & 4.573 & 0.166 & 0.913 & \textbf{84} \\
2  & \textbf{TC（总胆固醇）}  & 0.436 & 0.162 & 3.028 & 0.155 & 0.916 & \textbf{80} \\
3  & \textbf{血尿酸}          & 0.204 & 0.172 & 0.725 & 0.071 & 0.808 & \textbf{75} \\
4  & \textbf{HDL-C}           & 0.166 & 0.053 & 0.714 & 0.000 & 0.822 & \textbf{69} \\
5  & \textbf{LDL-C}           & 0.184 & 0.043 & 0.640 & 0.022 & 0.785 & \textbf{67} \\
6  & 空腹血糖                 & 0.040 & 0.000 & 0.097 & 0.000 & 0.407 & 49 \\
7  & ADL 穿衣                 & 0.027 & 0.014 & 0.159 & 0.000 & 0.522 & 48 \\
8  & ADL 吃饭                 & 0.002 & 0.014 & 0.119 & 0.000 & 0.464 & 43 \\
9  & BMI                      & 0.027 & 0.000 & 0.065 & 0.000 & 0.337 & 41 \\
10 & IADL 做饭                & 0.008 & 0.000 & 0.196 & 0.000 & 0.570 & 37 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{分析}：预警高血脂发病风险的 Top-5 指标为 \textbf{TG、TC、血尿酸、HDL-C、LDL-C}，全部为血常规血脂／代谢相关指标，与《中国成人血脂异常防治指南》的核心参数完全一致。

值得注意：TG 的 5 项方法得分\textbf{全部名列前茅}（Spearman 第 1、MI 第 1、LASSO 第 1、RF perm 第 1、Wald 第 2），Borda 得分 84 $=$ $4 \times 17 + 16$ 接近满分 85，说明 5 种互补方法一致地把 TG 选为最强预警指标，证实了 Borda 聚合的稳健性。
...
旧版仅用单变量 $\beta$、多元 $\beta$、MI、RF 四项 Z-score 加和，\textbf{没有计算效应量}。本文重新设计五项指标组成 Borda 聚合：
...
\subsubsection{关键发现与解释（对旧版\textbf{错误逻辑}的修正）}\label{sec:q1-tizhi-discussion}

旧版论文表 4 的核心逻辑是:"痰湿体质人群的平均痰湿积分高达 59.9（远超其他体质约 22 分）"，但这其实是\textbf{定义同义反复}：王琦九体质辨识标准本来就规定痰湿积分 $\ge$ 某阈值者被判定为痰湿体质，所以``痰湿体质人群痰湿积分高''是分类定义的必然后果，并非实证发现。本文予以修正，代之以具统计意义的\textbf{剂量---响应分析}：
...
\item \textbf{问题一方法论严格}：候选特征严格限定题目范围（血常规 + 活动量表），用 VIF 诊断去除完全共线汇总项，采用``参数/非参 $\times$ 线性/非线性 $\times$ 单变量/控制其它''三维正交的 5 方法，用 Borda 排序聚合代替量纲敏感的 Z-score 加和；
\item \textbf{纠正痰湿体质贡献度的逻辑错误}：以剂量--响应分析与效应量（Cohen's $h$）代替自证同义反复的``痰湿人群痰湿积分高''；
...
\subsection{问题一：关键指标筛选与体质贡献度分析（\texttt{Q1\_feature\_selection.py}）}
