...
## TODO:
1. 痰湿积分动力学里对于降低积分：频率达到5的阈值才开始有效，一级强度是否也对降低积分有3%的效果还是说从一级强度提到二级才会降3%？对此进行判断并给出相应的修改
2. 最优化方案：预算成本的权重是怎么来的？可不可以对题目理解如下：应以最大程度降低痰湿积分为主要目标，预算仅为边界约束条件，不应作为双目标去进行优化。在积分降低程度相同时再去选择预算低的方案。若二者都相同则选择时间成本最小的？这里你再斟酌一下题目的意思
3. 在这六个月中如果过完某个月后所选方案是不是可以改变？不用选一个方案然后去一直按这个走六个月，而是看付费的时间频率，中医调理分级月与月之间可以更换分级，然后活动干预我觉得也可以进行改变（按什么频率和强度我看题目的意思是可以随时改变）
4. 再仔细审阅题目，看看有没有其他的隐藏条件或者信息，现在修改目前论文题目三相应代码及文件，并处理由此带来的后续分析影响与结论。
